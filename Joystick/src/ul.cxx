...
ulDirEnt *ulReadDir(ulDir *dir) {
  ...
  strcpy(dir->curr.d_name, (const char *)dir->data.cFileName);
#else
...
char *ulMakePath(char *path, const char *dir, const char *fname) {
  if (fname) {
    if (!ulIsAbsolutePathName(fname) && dir != NULL && dir[0] != '\0') {
      strcpy(path, dir);
      strcat(path, SLASH);
      strcat(path, fname);
    } else
...
static int recursiveFindFileInSubDirs(char *filenameOutput, const char *tPath, const char *tfnameInput) {
  int bFound = FALSE;
  ...
  if (dirp != NULL) {
    ...
    while (!bFound && ((dp = ulReadDir(dirp)) != NULL)) {
      ...
      if (dp->d_isdir && strcmp(dp->d_name, ".") != 0 && strcmp(dp->d_name, "..") != 0) {
...
int ulIsAbsolutePathName(const char *pathname) {
  ...
  */
  ...
  return (pathname[0] == SLASH[0] || pathname[0] == '/') ||
         (((pathname[0] >= 'a' && pathname[0] <= 'z') || (pathname[0] >= 'A' && pathname[0] <= 'Z')) &&
          pathname[1] == ':' && (pathname[2] == SLASH[0] || pathname[2] == '/'));
