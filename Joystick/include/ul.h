...
#define PLIB_VERSION (PLIB_MAJOR_VERSION * 100 + PLIB_MINOR_VERSION * 10 + PLIB_TINY_VERSION)
...
inline void ulEndianSwap(unsigned short *x) { *x = ((*x >> 8) & 0x00FF) | ((*x << 8) & 0xFF00); }
...
inline void Reverse2Byte(uInt16 *data, int number) {
  BYTE b1, b2;
  for (int i = 0; i < number; ++i) {
    b1 = *data >> 8;     // word & 0xf0;//<< 4;
    b2 = *data & 0x00ff; //<<8;
...
inline void Reverse4Byte(uInt32 *data, int number) {
  uInt16 w1, w2;
  for (int i = 0; i < number; ++i) {
    w1 = *data >> 16;
    w2 = *data & 0x0000ffff;
...
public:
  ulDynamicLibrary(const char *libname) {
    ...
    if (sz < 64) {
      ...
      error = GetSharedLibrary(pstr, kPowerPCCFragArch, kReferenceCFrag, &connection, NULL, NULL);
    } else
      error = 1;
  }
...
public:
  ...
  void *getFuncAddress(const char *funcname) {
    ...
    if (handle && get_image_symbol(handle, "funcname", B_SYMBOL_TYPE_TEXT, &sym) == B_NO_ERROR)
      return sym;
...
public:
  ...
  void *getFuncAddress(const char *funcname) { return (handle == NULL) ? NULL : dlsym(handle, funcname); }
...
public:
  ...
  void removeEntity(void *entity) { removeEntity(searchForEntity(entity)); }

  virtual void replaceEntity(unsigned int n, void *new_entity);

  void replaceEntity(void *old_entity, void *new_entity) { replaceEntity(searchForEntity(old_entity), new_entity); }
